...
class HourglassNet(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        for ind in range(self.num_stacks):
            ...
            # 特征选择和特征融合模块
            # 首先全局池化
            low_feat = nn.AdaptiveAvgPool2d(inter_feat)
            high_feat = nn.AdaptiveAvgPool2d(out_feat)
            # 获取全局特征
            global_feat = torch.cat([low_feat, high_feat])
            # 学习不同层的重要性
            weights = self.atten_layer(global_feat)
            # 特征融合
            low_feat = inter_feat * weights
            high_feat = out_feat * weights
            out_feat = low_feat + high_feat
            # 特征选择
            # 通道注意力
            chanel = nn.AdaptiveMaxPool2d(out_feat)
            ca = self.chanel_layer(chanel)
            # 扩展维度
            ca_plus = torch.unsqueeze(ca).repeat()
            # 平面注意力
            sa = self.sa_layer(out_feat)
            # 扩展维度
            sa_plus = torch.unsqueeze(sa).repeat()
            # 得到最终的结果
            atten = sa_plus + ca_plus
            out_feat = out_feat * atten
