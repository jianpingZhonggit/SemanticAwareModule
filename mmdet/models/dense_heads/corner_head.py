...
class CornerHead(BaseDenseHead):
    ...
    def forward_single(self,
        ...
        # import numpy as np
        # tl_img = tl_heat[0].sigmoid().permute(1,2,0).clone().cpu().numpy()
        # np.save('tl_heat_pred.npy', tl_img)
        # flag = (tl_img[:, :, 0] + tl_img[:, :, 1] + tl_img[:, :, 2]) < 0.1
        # tl_img[flag, :] = (1, 1, 1)
        # import cv2
        # cv2.imwrite('tl_heatmap.png', tl_img * 255)
        br_pool = self.br_pool[lvl_ind](x)
        br_heat = self.br_heat[lvl_ind](br_pool)
        # br_img = br_heat[0].sigmoid().permute(1,2,0).clone().cpu().numpy()
        # np.save('br_heat_pred.npy', br_img)
        # flag = (br_img[:, :, 0] + br_img[:, :, 1] + br_img[:, :, 2]) < 0.1
        # br_img[flag, :] = (1, 1, 1)
        # cv2.imwrite('br_heatmap.png', br_img*255)
        # exit(0)
    ...
    def loss_by_feat(
        ...
        # if img_name == 'M1009_img000095.jpg':
        #     tl_heat = targets['topleft_heatmap']
        #     tl_img = tl_heat[0].sigmoid().permute(1,2,0).clone().cpu().numpy()
        #     import numpy as np
        #     np.save('./tl_heat_gt.npy', tl_img)
        #     flag = (tl_img[:, :, 0] + tl_img[:, :, 1] + tl_img[:, :, 2]) < 0.1
        #     tl_img[flag, :] = (1, 1, 1)
        #     import cv2
        #     cv2.imwrite('tl_heatmap_gt.png', tl_img * 255)
        #     br_heat = targets['bottomright_heatmap']
        #     tl_img = br_heat[0].sigmoid().permute(1,2,0).clone().cpu().numpy()
        #     np.save('./br_heat_gt.npy', tl_img)
        #     flag = (tl_img[:, :, 0] + tl_img[:, :, 1] + tl_img[:, :, 2]) < 0.1
        #     tl_img[flag, :] = (1, 1, 1)
        #     # import cv2
        #     cv2.imwrite('br_heatmap_gt.png', tl_img * 255)
            # import cv2
    ...
    def loss_by_feat_single(self, tl_hmp: Tensor, br_hmp: Tensor,
        ...
        tl_img = gt_tl_hmp[0].permute(1,2,0).clone().cpu().numpy()
        import numpy as np
        np.save('./tl_heat_gt.npy', tl_img)
        gt_br_hmp = targets['bottomright_heatmap']
        tl_img = gt_br_hmp[0].permute(1,2,0).clone().cpu().numpy()
        import numpy as np
        np.save('./br_heat_gt.npy', tl_img)
    ...
    def _decode_heatmap(self,
        ...
        import numpy as np
        tl_img = tl_heat[0].permute(1,2,0).clone().cpu().numpy()
        np.save('tl_heat_pred.npy', tl_img)
        tl_img = br_heat[0].permute(1,2,0).clone().cpu().numpy()
        np.save('br_heat_pred.npy', tl_img)
